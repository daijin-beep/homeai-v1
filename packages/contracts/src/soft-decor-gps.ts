...
export const VerifiedSkuSchema = z
  ...
  .superRefine((sku, ctx) => {
    if (!sku.imageUri.startsWith("fixture://")) {
      ctx.addIssue({
        code: "custom",
        message: "VerifiedSku imageUri must use a local fixture URI",
        path: ["imageUri"]
      });
    }
    if (!sku.leadUri.startsWith("fixture://")) {
      ctx.addIssue({
        code: "custom",
        message: "VerifiedSku leadUri must use a local fixture URI",
        path: ["leadUri"]
      });
    }
    if (sku.price.amount <= 0) {
      ctx.addIssue({
        code: "custom",
        message: "VerifiedSku price amount must be positive",
        path: ["price", "amount"]
      });
    }
  });
...
export const VerifiedSkuAdmissionResultSchema = z
  ...
  .superRefine((result, ctx) => {
    if (result.status === "admitted" && result.sku === undefined) {
      ctx.addIssue({ code: "custom", message: "admitted SKU requires sku", path: ["sku"] });
    }
    if (result.status === "rejected" && result.issues.length === 0) {
      ctx.addIssue({ code: "custom", message: "rejected SKU requires issues", path: ["issues"] });
    }
    if (result.status === "admitted" && result.issues.length > 0) {
      ctx.addIssue({ code: "custom", message: "admitted SKU must not carry blocking issues", path: ["issues"] });
    }
  });
...
export const VerifiedSkuCatalogSchema = z
  ...
  .superRefine((catalog, ctx) => {
    if (catalog.totalRawSkus !== catalog.results.length) {
      ctx.addIssue({ code: "custom", message: "totalRawSkus must match results", path: ["totalRawSkus"] });
    }
    if (catalog.admittedCount !== catalog.verifiedSkus.length) {
      ctx.addIssue({ code: "custom", message: "admittedCount must match verifiedSkus", path: ["admittedCount"] });
    }
    if (catalog.rejectedCount !== catalog.results.filter((result) => result.status === "rejected").length) {
      ctx.addIssue({ code: "custom", message: "rejectedCount must match rejected results", path: ["rejectedCount"] });
    }
  });
...
export type VerifiedSkuSize = z.infer<typeof VerifiedSkuSizeSchema>;
export type RawSkuFixture = z.infer<typeof RawSkuFixtureSchema>;

...
const forbiddenCodePoints = new Set([
  0x061c, 0x200b, 0x200c, 0x200d, 0x200e, 0x200f, 0x2028, 0x2029, 0x202a,
  0x202b, 0x202c, 0x202d, 0x202e, 0x2066, 0x2067, 0x2068, 0x2069, 0xfeff,
]);

const batch18RawGuardFiles = [
  { path: "apps/web/app/beta/page.tsx", expectedLfBytes: 10, minLfBytes: 5 },
  { path: "apps/web/app/page.tsx", expectedLfBytes: 58, minLfBytes: 30 },
  {
    path: "apps/web/components/v1-beta-flow/V1BetaFlowShell.tsx",
    expectedLfBytes: 375,
    minLfBytes: 250,
  },
  {
  ...
  {
    path: "packages/contracts/src/v1-beta-flow.ts",
    expectedLfBytes: 132,
    minLfBytes: 80,
  },
  {
    path: "tests/contracts/v1-beta-flow-contracts.test.ts",
    expectedLfBytes: 142,
    minLfBytes: 90,
  },
  {
    path: "tests/frontend/v1-beta-flow-shell.test.tsx",
    expectedLfBytes: 66,
    minLfBytes: 30,
  },
  {
    path: "tests/scope/v1-beta-flow-scope-scan.test.ts",
    expectedLfBytes: 208,
    minLfBytes: 120,
  },
];
...
describe("V1 Beta flow shell scope scan", () => {
  it("keeps Batch 18 out of ADS runtime, provider, live network, commerce, and Space Truth write paths", () => {
    ...
    const forbidden = [
      ...
      {
        pattern:
          /OPENAI_API_KEY|ANTHROPIC_API_KEY|GOOGLE_API_KEY|DASHSCOPE_API_KEY/i,
        allow: [],
      },
      { pattern: /sku/i, allow: [] },
      ...
      {
        pattern: /construction|contractor|load-bearing|structural/i,
        allow: [],
      },
      {
        pattern:
      ...
      return lines.flatMap((line, index) =>
        forbidden
          .filter(
            ({ pattern, allow }) =>
              pattern.test(line) &&
              !allow.some((allowed) => allowed.test(line)),
          )
  ...
  it.each(batch18RawGuardFiles)(
    "$path uses strict Batch 18 ASCII LF raw formatting",
    ({ path, expectedLfBytes, minLfBytes }) => {

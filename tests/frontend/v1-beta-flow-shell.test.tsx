...
describe("V1 Beta flow shell", () => {
  it("renders deterministic flow stages and Scheme Page status", () => {
    ...
    expect(screen.getByTestId("v1-beta-flow-status")).toHaveTextContent("needs_review");
  ...
  it("exposes guardrails as false for runtime and provider paths", () => {
    ...
    expect(screen.getByTestId("v1-beta-flow-guardrails")).toHaveTextContent("adsRuntimeConsumed");
    expect(screen.getByTestId("v1-beta-flow-guardrails")).toHaveTextContent("false");
    expect(screen.getByTestId("v1-beta-flow-guardrails")).toHaveTextContent("deterministicFixturesOnly");
    expect(screen.getByTestId("v1-beta-flow-guardrails")).toHaveTextContent("true");
  });

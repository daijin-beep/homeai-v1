...
describe("Verified SKU admission", () => {
  it("imports only admitted local fixture SKUs into the verified catalog", () => {
    ...
    expect(catalog.totalRawSkus).toBe(localRawSkuFixtures.length);
    expect(catalog.verifiedSkus.map((sku) => sku.skuId)).toEqual([
      "sku-local-rug-001",
      "sku-local-sofa-001"
    ]);
    expect(catalog.verifiedSkus.every((sku) => sku.size.widthMm > 0 && sku.size.depthMm > 0 && sku.size.heightMm > 0))
      .toBe(true);
  });
  ...
  it("fails closed for malformed raw fixtures", () => {
    ...
    expect(result.issues.map((issue) => issue.code)).toEqual(["invalid_payload"]);
  });

  it("builds a deterministic debug catalog", () => {
    const catalog = buildVerifiedSkuDebugFixture();

    expect(catalog.importedAt).toBe("2026-05-16T00:00:00.000Z");
